 ...
 */
public class ZumbugEntity extends BasePinataEntity {
    public ZumbugEntity(EntityType<? extends Animal> type, Level level) { super(type, level); baseCandyCount = 6; }
    ...
    @Override public String getPinataSpeciesName() { return "Zumbug"; }
    @Override public List<ItemStack> getVisitFoods() { return List.of(new ItemStack(Items.APPLE), new ItemStack(Items.CARROT)); }
    ...
    @Override public List<ItemStack> getCandyDrops() { return List.of(new ItemStack(ModPinataItems.ZUMBUG_CANDY.get(), 1 + random.nextInt(baseCandyCount))); }
    ...
    @Nullable @Override public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob other) {
        ZumbugEntity baby = ModPinataEntities.ZUMBUG.get().create(level);
