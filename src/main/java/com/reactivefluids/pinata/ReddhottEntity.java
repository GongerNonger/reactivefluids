 ...
 */
public class ReddhottEntity extends BasePinataEntity {
    public ReddhottEntity(EntityType<? extends Animal> type, Level level) { super(type, level); baseCandyCount = 3; }
    ...
    @Override public String getPinataSpeciesName() { return "Reddhott"; }
    @Override public List<ItemStack> getVisitFoods() { return List.of(new ItemStack(Items.BLAZE_POWDER), new ItemStack(Items.GLOWSTONE_DUST)); }
    ...
    @Override public List<ItemStack> getCandyDrops() { return List.of(new ItemStack(ModPinataItems.REDDHOTT_CANDY.get(), 1 + random.nextInt(baseCandyCount))); }
    ...
    @Nullable @Override public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob other) {
        ReddhottEntity baby = ModPinataEntities.REDDHOTT.get().create(level);
