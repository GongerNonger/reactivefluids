...
/**
 * A raised skeleton minion — fights with its bow for the caster until destroyed.
 * Created by casting Raise Dead on stacked bone blocks.
 */
public class RaisedSkeletonEntity extends Skeleton {
    ...
    private static final EntityDataAccessor<Integer> DATA_TICKS_REMAINING =
            SynchedEntityData.defineId(RaisedSkeletonEntity.class, EntityDataSerializers.INT);

    private UUID ownerUUID;

    public RaisedSkeletonEntity(EntityType<? extends Skeleton> type, Level level) {
        super(type, level);
    }
    ...
    protected void registerGoals() {
        // Skeleton's default goals include RangedBowAttackGoal via reassessWeaponGoal()
        super.registerGoals();
        // Add follow-owner goal at lower priority than combat
        this.goalSelector.addGoal(5, new FollowOwnerGoal());

        // Replace target selector — attack monsters, not players
    ...
    public void tick() {
        ...
        if (!level().isClientSide()) {
            ...
            if (remaining <= 100 && remaining % 20 == 0) {
                level().playSound(null, this, SoundEvents.SKELETON_AMBIENT,
